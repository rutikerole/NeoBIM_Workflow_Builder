...
function matchTemplate(text: string): WorkflowTemplate {
  ...
  if (q.includes("variant") || q.includes("options"))                    return PREBUILT_WORKFLOWS.find(w => w.id === "wf-07") ?? PREBUILT_WORKFLOWS[0];
  if (q.includes("image") && q.includes("concept"))                      return PREBUILT_WORKFLOWS.find(w => w.id === "wf-08") ?? PREBUILT_WORKFLOWS[0];
  if (q.includes("compliance") || q.includes("zoning"))                  return PREBUILT_WORKFLOWS.find(w => w.id === "wf-06") ?? PREBUILT_WORKFLOWS[0];
