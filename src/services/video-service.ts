...
export function buildFloorPlanCombinedPrompt(_buildingDescription: string, _roomInfo?: string): string {
  ...
  return "Transform this 2D architectural floor plan into an ultra-realistic architectural render. Start with a dramatic aerial reveal of the completed building exterior — a modern minimalist single-story residence with clean white facades, floor-to-ceiling glass windows, flat roof with subtle overhangs, and a landscaped entrance. Hold the exterior for 3 seconds showing the building from a 45-degree elevated angle. Then the camera smoothly pushes through the front entrance and transitions into a seamless interior walkthrough at eye level — gliding through the living room with polished concrete floors and minimal furniture, past the open dining area with a sleek table, into the modern white kitchen with handleless cabinets. The camera continues along the hallway into each bedroom with soft natural light pouring through large windows. Every room layout, wall position, door placement, and spatial proportion must match the floor plan exactly. Modern minimalist interior design, white walls, glass and steel details, warm natural daylight, volumetric light rays, ultra-photorealistic materials, V-Ray render quality, 8K architectural visualization, smooth cinematic dolly movement, 10 seconds.";
}
